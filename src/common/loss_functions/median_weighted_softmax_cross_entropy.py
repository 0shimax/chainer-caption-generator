...
sysm60k_weights =  \
  [0.010080947, 0.030313275, 0.083410853, 0.117724289, 0.172990354, 1.0,
    2.561904762, 3.684931507, 5.847826087, 29.88888889, 38.42857143,
    0.040053603, 0.055948419, 0.064169847, 0.2858661, 1.345, 29.88888889,
    269, 269]

class_wieghts_dict = {'sysm60k':numpy.array(sysm60k_weights, dtype=numpy.float32), \
                     }
...
class MedianWeightedSoftmaxCrossEntropy(function.Function):

    """Softmax activation followed by a cross entropy loss."""

    ignore_label = -1

    def __init__(self, use_cudnn=True, normalize=True, cache_score=True, weight_type='sysm60k'):
        self.use_cudnn = use_cudnn
        self.normalize = normalize
        self.cache_score = cache_score
        self.class_weights = class_wieghts_dict[weight_type]
    ...
    def forward_cpu(self, inputs):
        ...
        y = (log_p * (t.ravel() != self.ignore_label) * tw.ravel()).sum(keepdims=True) \
            * (-self._coeff)
        return y.reshape(()).astype(numpy.float32),
    ...
    def forward_gpu(self, inputs):
        ...
        ret = cuda.reduce(
            ...
            't == -1 ? 0 : log_y[_j * n_channel + t]*tw',
            'a + b', 'out = a * -coeff[0]', '0', 'crossent_fwd'
        )(t, cupy.array(tw), log_y.reduced_view(), log_y.shape[-1], self._coeff)
        return ret,
...
def median_weighted_softmax_cross_entropy(
    ...
    """
    return MedianWeightedSoftmaxCrossEntropy(use_cudnn, normalize, cache_score, weight_type)(x, t)
